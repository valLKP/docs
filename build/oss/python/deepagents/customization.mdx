...
<Note>
Skills require `deepagents>=0.4.1`.
</Note>
...
<Note>
Structured output for the main agent's `response_format` requires `deepagents>=0.5.3`.
</Note>
...
n and examples, see [response format](/oss/python/langchain/structured-output#response-format).
