...
<Note>
The current summarization behavior (in-model summarization via `wrap_model_call`, accurate token counting, and automatic `ContextOverflowError` fallback) was introduced in `deepagents` 0.4.0.
</Note>
...
<Note>
The summarization tool middleware requires `deepagents>=0.4.8`.
</Note>
